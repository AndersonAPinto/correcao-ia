...
export function verifyToken(token) {
  try {
    ...
    console.log(`[AUTH] verifyToken falhou — motivo: ${error.name} (${error.message})`);
    return null;
  }
}
...
export function getUserFromRequest(request) {
  ...
  if (!cookie?.value) {
    console.log('[AUTH] getUserFromRequest — cookie "session" ausente na requisição');
    return null;
  }

  console.log('[AUTH] getUserFromRequest — cookie "session" encontrado, verificando JWT...');
  const decoded = verifyToken(cookie.value);

  if (!decoded) {
    console.log('[AUTH] getUserFromRequest — JWT inválido ou expirado');
    return null;
  }

  console.log(`[AUTH] getUserFromRequest — userId extraído: ${decoded.userId}`);
...
/**
 * Gera token de restauração de conta (expira em 7 dias).
 * Enviado por email logo após a solicitação de exclusão.
 */
export function generateRestoreToken(userId) {
  return jwt.sign(
    { userId, type: 'account_restore' },
    SECRET_KEY,
    { expiresIn: '7d' }
  );
}
...
/**
 * Verifica token de restauração de conta.
 */
export function verifyRestoreToken(token) {
  try {
    const decoded = jwt.verify(token, SECRET_KEY);
    if (decoded.type !== 'account_restore') {
      return null;
    }
    return decoded;
  } catch (error) {
