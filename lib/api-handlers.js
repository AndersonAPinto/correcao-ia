...
/**
 * Ensures the authenticated user is active (not soft-deleted or pending deletion).
 * The middleware cannot check this (no DB access in Edge Runtime), so every
 * sensitive route must call this instead of plain requireAuth().
 *
 * Returns userId on success; throws on any invalid state.
 */
export async function requireActiveUser(request) {
  ...
  if (user.status === 'pending_deletion' || user.status === 'deleted') {
    console.log(`[API-HANDLERS] requireActiveUser — acesso bloqueado para conta ${user.status}: ${userId}`);
    const error = new Error('Esta conta foi desativada. Acesso negado.');
