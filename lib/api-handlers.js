...
export function isVertexAIConfigured() {
  ...
  // Verificar se o Project ID é um placeholder
  const isPlaceholder = !projectId ||
    projectId === 'seu-project-id-aqui' ||
    (projectId && projectId.toLowerCase().includes('seu-projeto')) ||
    (projectId && projectId.toLowerCase().includes('your-project'));
  ...
  if (credentialsEnv && credentialsEnv !== 'null' && credentialsEnv !== 'undefined') {
    try {
      ...
      if (isPlaceholder && credentials.project_id) {
        ...
        console.log(`ℹ️ [VERTEX AI] Project ID extraído do JSON via GOOGLE_CLOUD_CREDENTIALS: ${projectId}`);
      }
    } catch (error) {
      // Ignorar erro de parsing, continuar tentando outras fontes
    }
  }
  ...
  if (!credentials) {
    ...
    if (envCredentialsPath) {
      try {
        ...
        if (isPlaceholder && credentials.project_id) {
          projectId = credentials.project_id;
          console.log(`ℹ️ [VERTEX AI] Project ID extraído do arquivo: ${projectId}`);
        }
      } catch (error) {
  ...
  // Retornar true se temos um projectId válido (não placeholder) ou se temos credenciais
  return !!(projectId && !isPlaceholder) || !!credentials;
}

// Helper to get Vertex AI client
export function getVertexAIClient() {
  let projectId = process.env.GOOGLE_CLOUD_PROJECT_ID;
  const location = process.env.GOOGLE_CLOUD_LOCATION || 'us-east4'; // Alterado de us-central1 para us-east4 como alternativa estável
  const credentialsEnv = process.env.GOOGLE_CLOUD_CREDENTIALS;

  // Verificar se o Project ID é um placeholder
  const isPlaceholder = !projectId ||
    projectId === 'corregia' ||
    (projectId && projectId.toLowerCase().includes('corregia')) ||
    (projectId && projectId.toLowerCase().includes('corregia'));
  ...
  if (credentialsEnv && credentialsEnv !== 'null' && credentialsEnv !== 'undefined') {
    try {
      ...
      console.log('✅ [VERTEX AI] Usando credenciais via GOOGLE_CLOUD_CREDENTIALS');
  ...
  if (!credentials) {
    ...
    if (envCredentialsPath) {
      try {
        ...
        console.warn(`⚠️ [VERTEX AI] Usando arquivo de credenciais: ${path.split('/').pop()}`);
  ...
  if (!projectId || (isPlaceholder && !credentials)) {
  ...
  if (credentials && credentialsPathFound) {
    ...
    console.warn('⚠️ [VERTEX AI] Nenhuma credencial explícita encontrada. O SDK tentará usar Application Default Credentials (ADC).');
    console.warn('ℹ️ [VERTEX AI] Dica: No ambiente local, use `gcloud auth application-default login`');
  }
