...
export function getVertexAIClient() {
  ...
  const isPlaceholder = !projectId ||
    ...
    projectId.toLowerCase().includes('seu-projeto') ||
    projectId.toLowerCase().includes('your-project');
  ...
  if (!credentials) {
    ...
    if (envCredentialsPath) {
      possiblePaths.push(envCredentialsPath.startsWith('/') ? envCredentialsPath : resolve(process.cwd(), envCredentialsPath));
    }
...
export async function callGeminiAPIWithRetry(url, body, maxRetries = 3) {
  ...
  // Priorizando Gemini 2.0 conforme solicitado
  const modelosParaTentar = ['gemini-2.0-flash', 'gemini-2.0-flash-001', 'gemini-1.5-flash', 'gemini-1.5-pro'];
  let modeloAtualIndex = 0;

  // Determinar modelo inicial pela URL (priorizando 2.0 conforme solicitado)
  if (url && url.includes('gemini-2.0-flash')) {
    modeloAtualIndex = 0;
  } else if (url && url.includes('gemini-1.5-pro')) {
    modeloAtualIndex = 3;
