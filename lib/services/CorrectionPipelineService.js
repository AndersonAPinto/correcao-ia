...
/**
 * Re-executa apenas as Etapas 2 (Correção) e 3 (Análise) com texto OCR já disponível.
 * Usado quando o professor edita a transcrição e quer recorrigir sem refazer o OCR.
 */
export async function runDissertativaCorrectionAndAnalysis({
  textoOCR,
  questoesOCR = [],
  gabarito,
  habilidades = [],
  ...
  const correctionData = await runDissertativaCorrection({
    textoOCR,
    questoesOCR,
    gabaritoConteudo: gabarito.conteudo || '',
    disciplina,
  ...
  const habilidadesPontuacao = (correctionData.habilidades_avaliacao || [])
    .filter(h => h.habilidade_id && h.pontuacao)
    .map(h => ({ habilidadeId: h.habilidade_id, pontuacao: parseFloat(h.pontuacao) || 0, justificativa: h.justificativa || '' }));
  ...
  try {
    ...
    analysisData = { causa_raiz_erro: '', ponto_forte: '', ponto_atencao: '', sugestao_intervencao: '' };
  }

  return { correctionData, analysisData };
}

export default {
  runDissertativaPipeline,
  runMultiplaEscolhaPipeline,
  runDissertativaCorrectionAndAnalysis
};
